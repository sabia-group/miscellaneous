...
def parse_frame(rr, histo, rawdata, bottom_hydrogens, species, framecounter):
    ...
    bottom_h_tmp = (
        []
    )  # To have the 1st peak exactly, without overlap with the second one.
    ...
    for h in hydrogens:
        ...
        if d > zmax:
            raise RuntimeError(
                "Some atoms are above zmax (distance to surface %f Angstrom found),\nconsider readjusting histogram settings."
                % d
            )
        elif d < 0:
...
if len(sys.argv) == 1:
    print(
        "'g_z_from_xyz.py' takes 'simulation.pos_*.xyz' files, and calculates\n"
...
if (nskiplist == []) or (species is None) or (dirlist == []):
...
bottom_hydrogens = (
    []
)  # list of all bottom hydrogens' coordinates to have the 1st peak exactly, without overlap with the second one
assert zmax > zmin
...
for d, nskip in zip(dirlist, nskiplist):
    ...
    for file in sorted(os.listdir(".")):
        if fnmatch.fnmatch(file, "simulation.pos_*.xyz"):
            ...
            nbeads += 1
            histo, rawdata, bottom_hydrogens = parse_frame(
                rr, histo, rawdata, bottom_hydrogens, species, framecounter
            )
            nframes_local += 1
...
np.savetxt(
    "g_z_%s_%s.dat" % (species, os.path.split(dirlist[0])[-1]),
    np.c_[x_axis, histo],
    fmt="%.4f",
    delimiter="    ",
    ...
    % (species, " ".join(line.strip() for line in sys.argv), framecounter),
)
...
np.savetxt(
    "g_z_%s_%s.bottom_hydrogens.dat" % (species, os.path.split(dirlist[0])[-1]),
    bottom_hydrogens,
    #           fmt='    %.5f    %.5f    %.5f',
...
np.savetxt(
    "g_z_%s_%s.bottom_hydrogens.histo.dat" % (species, os.path.split(dirlist[0])[-1]),
