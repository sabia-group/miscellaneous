...
def parse_frame(rr, histo, rawdata, framecounter):
    ...
    d = com-z0
    if d > zmax:
        raise RuntimeError("Center of mass is above zmax (distance to surface %f Angstrom found),\nconsider readjusting histogram settings." % d)
    elif d < zmin:
        raise RuntimeError("Center of mass is below zmin (distance to surface %f Angstrom found),\nconsider readjusting histogram settings." % d)
...
if len(sys.argv) == 1:
    print("\'g_z_center-of-mass.py\' takes 'simulation.pos_*.xyz' files, and calculates\n"
          "g(z) distribution (z is z-coordinate) for the centers of mass,\n"
          "assuming that there are two molecules in appropriate format:\n"
          "\t6 carbons mol1\n"
          "\t6 carbons mol2\n"
...
np.savetxt('g_z_center-of-mass_%s.dat' % (os.path.split(dirlist[0])[-1]),
           np.c_[x_axis, histo],
           fmt='%.4f',
...
np.savetxt('g_z_center-of-mass_%s.rawdata.dat'  % (os.path.split(dirlist[0])[-1]),
           rawdata[:,[0,1,3]],
           fmt='%i    %.5f    %.5f',
