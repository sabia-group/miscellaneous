...
def parse_frame(rr, histo, rawdata, framecounter):
    ...
    if(normale[2] < 0):
        normale = -normale
    angle_deg = 180 / np.pi * angle_between(normale, [0., 0., 1.])
...
print("\'angle-mol-surf.py\' takes 'simulation.pos_*.xyz' files, and calculates\n"
      "the distribution of angles between surface and cyclohexane molecules.\n"
      "It can parse several directories at once.\n")

if (len(sys.argv) == 1):
    print("Possibe arguments:\n"
...
np.savetxt('angles_%s.histo.dat' % (os.path.split(dirlist[0])[-1]),  
           np.c_[x_axis, histo], 
           fmt='%8.4g', 
           delimiter='    ', 
...
np.savetxt('angles_%s.rawdata.dat' % (os.path.split(dirlist[0])[-1]), 
           rawdata, 
           fmt='%i  %.5f', 
